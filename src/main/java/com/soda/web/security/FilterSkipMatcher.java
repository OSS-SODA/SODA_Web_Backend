...
public class FilterSkipMatcher implements RequestMatcher {
    ...
    public FilterSkipMatcher(List<String> pathToSkip, List<String> processingPath){
        this.orRequestMatcher =
                new OrRequestMatcher(pathToSkip.
                        stream().map(AntPathRequestMatcher::new).collect(Collectors.toList()));
        this.processingMatcher = new OrRequestMatcher(processingPath.stream()
